...
export function NewTransactionModal({isOpen, onRequestClose}: NewTransactionModaProps) {
    ...
    async function handleCreateNewTransactionModal(event: FormEvent) {
        ...
        onRequestClose();
        //fechar a modal após a finalizaão da inserção
    ...
    return(
        ...
        <Container onSubmit={handleCreateNewTransactionModal}>
            ...
            placeholder="Valor"
            value={amount}
            onChange={event => setAmount(Number(event.target.value))} //Uma função que executa, toda vez que um novo texto for digitado no input
            />
